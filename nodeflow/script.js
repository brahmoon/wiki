...
function collectLabelsByType(type, nodes = state.nodes, labels = []) {
  nodes.forEach(item => {
    if (item.type === type) {
      labels.push(item.label);
    }
    if (item.children.length) {
      collectLabelsByType(type, item.children, labels);
    }
  });
  return labels;
}

function ensureUniqueNodeLabels(node, usedLabels = null) {
  const labelsSet = usedLabels instanceof Set
    ? usedLabels
    : new Set(collectLabelsByType('node'));

  if (node.type === 'node') {
    const { base } = splitIndexedLabel(node.label);
    const uniqueLabel = generateIndexedLabel(base, labelsSet);
    node.label = uniqueLabel;
    labelsSet.add(uniqueLabel);
  }

  node.children.forEach(child => {
    ensureUniqueNodeLabels(child, labelsSet);
  });
}
...
function createNodeItem(parentId = null) {
  const parent = parentId ? findNodeById(parentId) : null;
  const siblings = parent ? parent.node.children : state.nodes;
  const label = generateIndexedLabel('Node', collectLabelsByType('node'));
  const newNode = {
    id: nextId('node'),
    label,
    type: 'node',
...
function duplicateNode(nodeId) {
  ...
  const usedNodeLabels = new Set(collectLabelsByType('node'));
  if (node.type === 'node') {
    ensureUniqueNodeLabels(clone, usedNodeLabels);
  } else {
    const { base } = splitIndexedLabel(node.label);
    const labels = siblings.map(item => item.label);
...
function bindEvents() {
  ...
  elements.sidebar.addEventListener('contextmenu', handleSidebarContextMenu);
