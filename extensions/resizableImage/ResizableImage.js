...
export const ResizableImage = Image.extend({
  ...
  addAttributes() {
    return {
      ...
      aspectRatio: {
        default: null,
        parseHTML: element => sanitizeAspectRatio(element.getAttribute('data-aspect-ratio')),
        renderHTML: attributes => {
          const ratio = sanitizeAspectRatio(attributes.aspectRatio);
          if (!ratio) {
            return {};
          }

          return { 'data-aspect-ratio': String(ratio) };
        },
      },
    };
  },
    ...
    return {
      ...
      setImageSize: options => ({ tr, state, dispatch }) => {
        ...
        state.doc.nodesBetween(from, to, (node, pos) => {
          ...
          const storedAspectRatio = getAspectRatioFromNode(node);
          let aspectRatio = storedAspectRatio;

          if (!aspectRatio && Number.isFinite(currentWidth) && Number.isFinite(currentHeight) && currentHeight > 0) {
            aspectRatio = sanitizeAspectRatio(currentWidth / currentHeight);
          }

          if (!aspectRatio && Number.isFinite(targetWidth) && Number.isFinite(targetHeight) && targetHeight > 0) {
            aspectRatio = sanitizeAspectRatio(targetWidth / targetHeight);
          }
          ...
          if (Number.isFinite(targetWidth) && Number.isFinite(targetHeight)) {
            const widthDerivedHeight = targetWidth / aspectRatio;
            const heightDerivedWidth = targetHeight * aspectRatio;
            const widthDiff = Math.abs(heightDerivedWidth - targetWidth);
            const heightDiff = Math.abs(widthDerivedHeight - targetHeight);

            if (!Number.isFinite(targetHeight) || heightDiff <= widthDiff) {
              targetHeight = widthDerivedHeight;
            } else {
              targetWidth = heightDerivedWidth;
            }
          } else if (Number.isFinite(targetWidth)) {
            targetHeight = targetWidth / aspectRatio;
          } else if (Number.isFinite(targetHeight)) {
          ...
          const previousAspectRatio = sanitizeAspectRatio(attrs.aspectRatio);
          const nextAspectRatio = sanitizeAspectRatio(nextWidth / nextHeight);

          if (previousWidth === nextWidth && previousHeight === nextHeight && previousAspectRatio === nextAspectRatio) {
            return true;
          }
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      const applyNodeAttributes = attrs => {
        ...
        const ignoredKeys = new Set(['src', 'alt', 'title', 'width', 'height', 'aspectRatio']);
        Object.entries(attrs).forEach(([key, value]) => {
      ...
      const commitSize = (width, height) => {
        ...
        if (nextWidth === previousWidth && nextHeight === previousHeight && previousAspectRatio === nextAspectRatio) {
          return;
        }
      ...
      const validateAndCommitDimensions = () => {
        ...
        const aspectRatioAttr = sanitizeAspectRatio(currentNode.attrs.aspectRatio);
        const constraints = getConstraints();
        const maxEditorWidth = getEditorContentWidth(editor);
        ...
        if (Number.isFinite(widthAttr) && Number.isFinite(heightAttr) && widthAttr > 0 && heightAttr > 0) {
          ...
          if (!aspectRatioAttr) {
            commitSize(widthAttr, heightAttr);
          }
          return;
        }
        ...
        if (naturalWidth > 0 && naturalHeight > 0) {
          commitSize(naturalWidth, naturalHeight);
          return;
        }
      ...
      if (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') {
        ...
        setTimeout(validateAndCommitDimensions, 0);
      }
      ...
      return {
        ...
        destroy: () => {
          ...
          if (loadListener) {
            imageEl.removeEventListener('load', loadListener);
          }
        },
      };
    };
