...
4. Clicking **保存** sends an `updateDriveImageAuthorities` request. The Apps Script validates admin access, writes the data to the `Authorities` sheet, and returns the normalized record set. Successful admin responses now echo the latest `adminToken` and `adminTokenExpiresAt` values, enabling the client to reuse short-lived session tokens without prompting for credentials on every call.
